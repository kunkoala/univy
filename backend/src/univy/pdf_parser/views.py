...
@router.delete("/clear_documents")
async def clear_documents(lightrag_client: LightRAGClient = Depends(get_lightrag_client)):
    result = await lightrag_client.clear_documents()
    if result.get("status") == "error" or result.get("status") == "fail":
        raise HTTPException(status_code=500, detail=result.get("message", "Failed to clear documents"))
    return result
